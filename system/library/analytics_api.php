...
//-------------------------------------------------------------------------------------------------
	...
	//---------------------------------------------------------------------------------------------
		...
		if($filters) {
			if(is_object($filters) && ( $filters instanceof analytics_filters) && $filters->filters) {
	...
	//---------------------------------------------------------------------------------------------
		...
		$data = $this->data($id, '', 'ga:visits,ga:pageviews,ga:timeOnSite,ga:newVisits', false, $start, $end, 10, 1, $filters, $debug);
		
		if(isset($data['ga:visits'])) {
			...
			$data['newVisitsPerc'] = round( ( $data['ga:newVisits'] / $data['ga:visits'] ) * 100, 2 );
		}
		else {
			$data['ga:visits'] = 0;
			$data['ga:newVisits'] = 0;
