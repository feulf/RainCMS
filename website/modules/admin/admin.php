...
class AdminModule extends Module {
    ...
    function index() {
        View::configure("tpl_dir", THEMES_DIR . "admin" . "/");
        $this->loader->set_layout("layout.index");
        $this->content_edit();
//            View::configure( "tpl_dir", THEMES_DIR . $this->loader->theme . "/" );
    }

    function content_edit($content_id = null) {

        add_style( "css/content.css", $this->loader->theme_dir );
        ...
        if (!$content_id) {
            ...
            if ($type['file_enabled'])
                $edit_html .= '<div id="content_file">' . $this->_content_file_list($content_id) . '</div>' . "\n";
    ...
    //------------------------------------------
    // draw file list
    function _content_file_list($content_id) {
